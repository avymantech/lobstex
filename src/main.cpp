...
#include "zlobs/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "Lobstex cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 60 * 4;
int64_t nReserveBalance = 0;

/** Fees smaller than this (in ulobs) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zLOBS spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zLOBS is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zLOBS spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zLOBS without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zLOBS spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zLOBS spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zlobs");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zLOBS mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight >= 835000 && nHeight <= 835050) {
		nSubsidy = 5.5 * COIN;
		return nSubsidy;
	}
        ...
        nSubsidy = 30 * COIN;
    } else if (nHeight > 300000 && nHeight <= 560000) {
        nSubsidy = 15 * COIN;
    } else if (nHeight > 560000 && nHeight <= 1055300){
		nSubsidy = 6 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZLOBSStake)
{
    int64_t ret = 0;
    ...
    if (nHeight < 5000) {
        ...
        ret = blockValue * .50;
    } else if (nHeight > 43200 && nHeight <= 573080) {
        ret = blockValue * .60;
    } else if (nHeight > 573080 && nHeight <= 1055300) {
        ret = 4 * COIN;
	} else if (nHeight > 1055300 && nHeight <= 1581600) {
        ret = 3 * COIN;
	} else if (nHeight > 1581600) {
...
int64_t GetTreasuryAward(int nHeight)
{
	if(IsTreasuryBlock(nHeight)) {
		if (nHeight == nStartTreasuryBlock) {
			return 1441 * COIN; //4320 for the first treasury block
		} else if (nHeight <= 1055300) {
			return 1441 * COIN; //4320 for each next block 
		} else if (nHeight <= 1581600) {
			return 1441 * COIN; //4320 for each next block 
	 	} else if (nHeight >= 1581600) {
			return 720.5 * COIN; //4320 for each next block 
		} else {
		return 0;
		}
	}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Lobstex
...
void RecalculateZLOBSMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZLOBS..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZLOBS..."), percent);
        }
...
void RecalculateZLOBSSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZLOBS..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZLOBS..."), percent);
        }

        //Rewrite zLOBS supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zLOBS supply
...
bool RecalculateLOBSSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating LOBS supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating LOBS supply..."), percent);
        }
...
bool UpdateZLOBSSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zLOBS supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateLOBSSupply(Params().Zerocoin_StartHeight());
    }

    //Track zLOBS money supply in the block index
    if (!UpdateZLOBSSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zLOBS supply for block=%s height=%d", __func__,
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert LOBS to zLOBS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zLOBS spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zLOBS serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zLOBS stake block is less than allowed start height", __func__);

    if (CZPivStake* zLOBS = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zLOBS->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zLOBS stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zLOBS stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zLOBS->GetValue()));
        if (nChecksum200 != zLOBS->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zLOBS->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZLOBS() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zLOBS fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> lobsInputs;
        std::vector<CTxIn> zLOBSInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zLOBSInputs.push_back(stakeIn);
            }else{
                lobsInputs.push_back(stakeIn);
            }
        }
        const bool hasLOBSInputs = !lobsInputs.empty();
        const bool hasZLOBSInputs = !zLOBSInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasLOBSInputs)
                    ...
                    for (const CTxIn& lobsIn : lobsInputs){
                        if(lobsIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : lobsInputs) {
            ...
            // Now that this loop if completed. Check if we have zLOBS inputs.
            if(hasZLOBSInputs){
                for (const CTxIn& zPivInput : zLOBSInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zlobs");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zLOBS mints and %d zLOBS spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Lobstex: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zlobs", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zlobs", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
