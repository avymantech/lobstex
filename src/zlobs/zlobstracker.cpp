...
CzLOBSTracker::CzLOBSTracker(std::string strWalletFile)
{
...
CzLOBSTracker::~CzLOBSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLOBSTracker::Init()
{
...
bool CzLOBSTracker::Archive(CMintMeta& meta)
{
...
bool CzLOBSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLOBSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLOBSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLOBSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLOBSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLOBSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLOBSTracker::GetSerialHashes()
{
...
CAmount CzLOBSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLOBSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLOBSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLOBSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLOBSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLOBSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLOBSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLOBSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLOBSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLOBSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLOBSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLOBSWallet* zLOBSWallet)
{
    bool iszLOBSWalletInitialized = (NULL != zLOBSWallet);
    CMintMeta meta;
    ...
    if (! iszLOBSWalletInitialized)
        zLOBSWallet = new CzLOBSWallet(strWalletFile);
    meta.isSeedCorrect = zLOBSWallet->CheckSeed(dMint);
    if (! iszLOBSWalletInitialized)
        delete zLOBSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLOBSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLOBSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLOBSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLOBSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLOBSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLOBSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLOBSWallet* zLOBSWallet = new CzLOBSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zLOBSWallet);
        }
        delete zLOBSWallet;
        LogPrint("zero", "%s: added %d dzlobs from DB\n", __func__, listDeterministicDB.size());
    }
