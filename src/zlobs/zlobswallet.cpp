...
CzLOBSWallet::CzLOBSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlobs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLOBSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLOBSSeed_deprecated()) {
                    LogPrintf("%s: Updated zLOBS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlobs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlobs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLOBSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLOBSCount(nCountLastUsed);
    else if (!walletdb.ReadZLOBSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLOBSWallet::Lock()
{
    seedMaster = 0;
}

void CzLOBSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLOBSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLOBS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLOBSWallet::LoadMintPoolFromDB()
{
...
void CzLOBSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLOBSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLOBSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlobsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLOBSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLOBS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlobsTracker which also adds to database
    pwalletMain->zlobsTracker->Add(dMint, true);
...
void CzLOBSWallet::SeedToZLOBS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLOBSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLOBSWallet::GenerateDeterministicZLOBS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLOBSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLOBSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLOBSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
