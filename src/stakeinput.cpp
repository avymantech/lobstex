...
// The zLOBS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zLOBS is a hash of the serial
...
{
    //Create an output returning the zLOBS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZLOBSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zLOBS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zLOBS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZLOBSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zLOBS output", __func__);
...
{
    CzLOBSTracker* zlobsTracker = pwallet->zlobsTracker.get();
    CMintMeta meta;
    if (!zlobsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zlobsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a LOBS stake is the outpoint
