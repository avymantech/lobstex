...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zlobsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zlobsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zLOBS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zLOBS seed used to mint this zLOBS is not the same as currently hold in the wallet";
            else
