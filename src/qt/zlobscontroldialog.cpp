...
#include "ui_zlobscontroldialog.h"
...
std::list<std::string> ZLOBSControlDialog::listSelectedMints;
...
ZLOBSControlDialog::~ZLOBSControlDialog()
{
    delete ui;
}

void ZLOBSControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLOBSControlDialog::updateList()
{
...
void ZLOBSControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLOBSControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLOBS_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLOBSControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLOBSControlDialog::GetSelectedMints()
{
...
void ZLOBSControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
