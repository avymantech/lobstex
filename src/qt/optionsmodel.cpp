...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LOBS);
    ...
    if (!settings.contains("nAnonymizeLobstexAmount"))
        settings.setValue("nAnonymizeLobstexAmount", 1000);

    nAnonymizeLobstexAmount = settings.value("nAnonymizeLobstexAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLobstexAmount"))
        SoftSetArg("-anonymizelobstexamount", settings.value("nAnonymizeLobstexAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in lobstex.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLobstexAmount:
            return QVariant(nAnonymizeLobstexAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLobstexAmount:
            nAnonymizeLobstexAmount = value.toInt();
            settings.setValue("nAnonymizeLobstexAmount", nAnonymizeLobstexAmount);
            emit anonymizeLobstexAmountChanged(nAnonymizeLobstexAmount);
            break;
        case CoinControlFeatures:
