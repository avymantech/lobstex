...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zlobsTracker->HasMintTx(hash))) {
            //zLOBS stake reward
            TransactionRecord sub(hash, nTime);
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZLOBS;
            ...
            for (std::vector<CTxOut>::size_type i = 1; i < wtx.vout.size(); ++i) {
                CTxDestination addr;
                isminetype mine = ISMINE_NO;
                if (i == 1) {
                    mine = wallet->IsMine(wtx.vout[i]);
                } else {
                    if (ExtractDestination(wtx.vout[i].scriptPubKey, addr) && IsMine(*wallet, addr))
                        mine = wallet->IsMine(wtx.vout[i]);
                }
...
bool IsZLOBSType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZLOBS:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZLOBS || type == TransactionRecord::MNReward) {
